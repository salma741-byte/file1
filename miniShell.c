...
t_builtin	g_builtin[] = 
{
	{.builtin_name = "echo", .foo=jn_echo},  
	{.builtin_name = "env", .foo=jn_env},    
	{.builtin_name = "exit", .foo=jn_exit},  
	{.builtin_name = NULL},                   
};


/**
 * jn_launch - Crée un processus enfant pour exécuter des commandes externes
...
/**
 * jn_execute - Exécute des commandes internes ou externes
 * @args: Tableau des arguments de la commande
 * Return: 1 pour continuer le shell, 0 pour le quitter
 */
void	jn_execute(char **args)
{
	int i;
...
/**
 * jn_split_line - Splits input line into tokens
 * @line: Input line to be tokenized
 * Return: Array of tokens or NULL on failure
 */
char	**jn_split_line(char *line)
{
	size_t bufsize;
...
int main(){
	...
	while((line = jn_read_line())){

		args = jn_split_line(line);	
		int i;
